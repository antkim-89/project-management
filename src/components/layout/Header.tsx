      ...
      <header className="h-16 bg-surface-container-low border-b border-outline-variant flex items-center justify-between px-6 shrink-0 relative z-50">
        ...
        <div className="flex items-center gap-8">
          <Link to="/" className="flex items-center gap-3 hover:opacity-80 transition-opacity">
            ...
            <span className="font-bold text-xl text-on-surface tracking-tight">ProManage</span>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="uppercase text-[10px] font-bold tracking-widest">{theme}</span>
          </button>

          {/* Language Selector */}
          <button className="flex items-center gap-2 px-3 h-10 rounded-lg text-on-surface-variant hover:bg-interaction-hover hover:text-on-surface transition-all active:scale-95" onClick={toggleLanguage}>
            <Globe className="w-4 h-4" />
            <span className="uppercase text-[10px] font-bold tracking-widest">{i18n.language.split('-')[0]}</span>
          </button>
            ...
            {isAuthenticated && user ? (
              <>
                ...
                >
                  ...
                  <span className="text-label-md font-bold text-on-surface mr-2 hidden md:block">{user.name}</span>
                </button>
                ...
                >
                  ...
                  <div className="p-2 bg-surface-container">
                    ...
                    >
                      ...
                      <span className="text-label-md font-bold">Profile Detail</span>
                    ...
                    >
                      ...
                      <span className="text-label-md font-bold">{t('common.logout')}</span>
                    </button>
                  </div>
                </BasePopover>
              </>
            ) : (
              <button className="btn-primary px-5 h-10" onClick={login}>
                <div className="flex items-center gap-2">
