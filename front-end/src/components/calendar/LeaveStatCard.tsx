...
export const LeaveStatCard: React.FC<LeaveStatCardProps> = ({
  ...
  const getVariantStyles = () => {
    switch (variant) {
      ...
      case 'neutral': return 'text-on-surface-variant bg-surface-container-high border-outline-variant';
  ...
  const getIconColor = () => {
    switch (variant) {
      ...
      case 'neutral': return 'text-on-surface-variant';
  ...
  return (
    <GlassCard className="p-4 flex flex-col h-full">
      <div className="flex justify-between items-start mb-3">
        <div className={cn("p-2 rounded-lg", getVariantStyles().split(' ').slice(1).join(' '))}>
      ...
      <div>
        ...
        <h3 className="text-3xl font-mono font-bold text-on-surface">{value}</h3>
      </div>
    </GlassCard>
  );
};
