...
export const ResourceCard: React.FC<ResourceCardProps> = ({
  ...
  const getStatusColor = () => {
    switch (status) {
      ...
      case 'on-project': return 'bg-primary';
  ...
  const getMatchIcon = () => {
    if (matchScore && matchScore >= 90) return <Star size={14} fill="currentColor" />;
    if (matchScore && matchScore >= 80) return <Zap size={14} fill="currentColor" />;
  ...
  return (
    <GlassCard className="rounded-xl overflow-hidden flex flex-col relative p-0 border-0">
      {matchScore && (
        <div className="absolute top-4 right-4 z-10">
          <div className="bg-secondary-container/20 border border-secondary text-secondary px-3 py-1 rounded-full flex items-center gap-1.5 backdrop-blur-md">
            {getMatchIcon()}
            <span className="font-mono text-[12px] font-bold">{matchScore} pts - {getMatchLabel()}</span>
          </div>
        </div>
      )}
      ...
      <div className="p-4 flex gap-4">
        ...
        <div className="flex-1">
          <div className="flex justify-between items-start">
            <div>
              <h4 className="text-lg font-bold text-on-surface leading-tight">{name}</h4>
              <p className="text-primary text-[11px] font-bold uppercase tracking-wider">{role}</p>
            </div>
            <div className="text-right">
              <p className="text-on-surface-variant text-[11px] font-bold uppercase">M/M Cost</p>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {skills.map(skill => (
              <span key={skill} className="bg-surface-container-highest px-2 py-0.5 rounded text-[11px] font-medium border border-outline-variant/30 text-on-surface-variant">
                {skill}
              </span>
            ))}
          </div>
      ...
      <div className="mt-auto border-t border-outline-variant/20 p-4 flex items-center justify-between bg-surface-container-highest/30">
        ...
        {status === 'on-leave' ? (
          <div className="text-[11px] font-bold text-error uppercase">Available {availableDate}</div>
        ) : (
