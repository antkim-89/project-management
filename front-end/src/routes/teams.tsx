...
function Teams() {
  ...
  const filteredUsers = (users as TeamMember[] | undefined)?.filter(u => {
  ...
  const orgLevels = uniqueRanksForOrg.map(rank => ({
    rankName: rank.name,
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 bg-surface animate-fade-in">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-outline-variant bg-surface-container/50">
                  <th className="p-4 text-label-caps font-bold text-on-surface-variant tracking-wider">Member</th>
                  <th className="p-4 text-label-caps font-bold text-on-surface-variant tracking-wider">Rank / Role</th>
                  <th className="p-4 text-label-caps font-bold text-on-surface-variant tracking-wider">Monthly Base Salary</th>
                  <th className="p-4 text-label-caps font-bold text-on-surface-variant tracking-wider">Skills</th>
                  <th className="p-4 text-label-caps font-bold text-on-surface-variant tracking-wider">Status</th>
                  <th className="p-4 text-label-caps font-bold text-on-surface-variant tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredUsers.length > 0 ? (
                  filteredUsers.map((member) => (
                    <tr key={member.id} className="border-b border-outline-variant/50 hover:bg-interaction-hover transition-colors">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          />
                          <div>
                            <div className="font-bold text-on-surface text-label-md">{member.name}</div>
                            <div className="text-label-sm text-on-surface-variant">{member.email}</div>
                          </div>
                        </div>
                      </td>
                      <td className="p-4">
                        <span className="text-label-md font-semibold text-on-surface">{member.rank?.name || 'Employee'}</span>
                      </td>
                      <td className="p-4">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {member.skills && member.skills.length > 0 ? (
                            member.skills.map((s, idx) => (
                              <span key={idx} className="px-2 py-0.5 bg-primary-container/10 border border-primary/20 text-primary text-[10px] font-bold rounded">
                                {s.skillSet?.name}
                              </span>
                            ))
                          ) : (
                            <span className="text-outline text-label-sm">No skills mapped</span>
                          )}
                        </div>
                      </td>
                      <td className="p-4">
          ...
          {orgLevels.length > 0 ? (
            <div className="flex flex-col items-center gap-14 py-4 relative w-full">
              {orgLevels.map((level, levelIdx) => (
                <div key={level.rankName} className="flex flex-col items-center w-full relative">
                  {/* Rank Badge */}
                  ...
                  <div className="flex flex-wrap justify-center gap-8 z-10">
                    {level.members.map((member) => (
                      ...
                      >
                        ...
                        <h4 className="text-headline-md font-bold text-on-surface group-hover:text-primary transition-colors">{member.name}</h4>
                        <p className="text-label-sm text-on-surface-variant uppercase tracking-wider mt-1 font-semibold">{member.rank?.name}</p>
                        <p className="text-body-sm text-outline mt-2 truncate max-w-full">{member.email}</p>
            ...
            <div className="text-on-surface-variant py-8">
              No members matched the criteria to construct an organization chart.
            </div>
          )}
        </div>
      )}
      ...
      <div className="mt-8 flex flex-col md:flex-row items-center justify-between gap-4">
        <p className="text-label-md text-on-surface-variant">Showing {filteredUsers.length} of {users?.length || 0} entries</p>
        <div className="flex items-center gap-1">
          ...
          <button className="w-10 h-10 flex items-center justify-center rounded border border-outline-variant text-on-surface-variant hover:bg-interaction-hover hover:text-on-surface transition-colors font-bold">12</button>
          <button className="w-10 h-10 flex items-center justify-center rounded border border-outline-variant text-on-surface-variant hover:bg-interaction-hover hover:text-on-surface transition-colors">
