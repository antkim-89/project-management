...
import { ChevronLeft, ChevronRight, Filter, Square, CheckSquare } from 'lucide-react'
...
export const TaskCalendarView: React.FC = () => {
  ...
  const allProjectsSelected = projects ? selectedProjectIds.length === projects.length || selectedProjectIds.length === 0 : true
        ...
        <div className="relative shrink-0">
          <button
            ...
            className={cn(
              ...
              !allProjectsSelected && "border-primary/50 text-primary bg-primary/5"
            )}
          >
            ...
            <>
              ...
              <div className="absolute right-0 mt-2 w-72 bg-surface-container border border-outline-variant rounded-2xl shadow-2xl p-4 z-50 animate-zoom-in">
                <div className="flex items-center justify-between pb-3 border-b border-outline-variant/30 mb-3">
                  <span className="text-xs font-bold text-on-surface-variant uppercase tracking-wider">프로젝트 필터</span>
                  <button 
                    onClick={handleToggleAllProjects}
                    className="text-[11px] font-bold text-primary hover:underline"
                  >
                    {projects && selectedProjectIds.length === projects.length ? '전체 해제' : '전체 선택'}
                  </button>
                </div>
                
                {projectsLoading ? (
                  <div className="py-6 text-center text-xs text-on-surface-variant">프로젝트 로딩 중...</div>
                ) : (
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            isSelected ? "text-primary font-bold" : "text-on-surface font-normal"
                          )}
                        >
                          {isSelected ? (
          ...
          {weekdays.map((day, idx) => (
            <div
              ...
              className={cn(
                ...
                idx === 0 ? "text-error" : idx === 6 ? "text-secondary" : "text-on-surface-variant"
              )}
            >
              {day}
            </div>
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  !cell.isCurrentMonth && "opacity-40 bg-surface-container-low/10",
                  cellIsToday && "bg-primary-container/10 ring-1 ring-primary/20",
                  isWeekend && cell.isCurrentMonth && "bg-surface-container-low/20"
                )}
              >
                {/* Date indicator */}
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={cn(
                          ...
                          isTaskDone ? "bg-primary" : isTaskInProgress ? "bg-secondary" : "bg-outline"
                        )} />
                        {task.title}
                      </div>
                    )
