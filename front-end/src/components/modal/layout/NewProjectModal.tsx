...
export function NewProjectModal({ isOpen, onClose }: NewProjectModalProps) {
  ...
  const handleRoleChange = (userId: string, role: string) => {
    setAllocations(allocations.map(a => a.userId === userId ? { ...a, role } : a))
  }
        ...
        <div className="flex flex-col gap-6 animate-fade-in max-h-[60vh] overflow-y-auto pr-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {usersLoading ? (
              ...
              filteredUsers.map((user) => {
                const isAllocated = !!allocations.find(a => a.userId === user.id)
                const currentAlloc = allocations.find(a => a.userId === user.id)
          ...
          {allocations.length > 0 && (
            <div className="flex items-center gap-2 p-3 bg-secondary/5 border border-secondary/20 rounded-lg text-secondary text-label-md font-bold mt-2">
              ...
              <span>Allocating {allocations.length} personnel to project `{title}`</span>
            </div>
          )}
        </div>
      )}
