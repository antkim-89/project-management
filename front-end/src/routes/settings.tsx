  ...
  return (
    <GlassCard className="flex items-center p-4 group">
      ...
      <div className="flex-1 ml-4">
        <h3 className="font-bold text-on-surface group-hover:text-primary transition-colors">{title}</h3>
        <p className="text-label-md text-on-surface-variant">{description}</p>
      </div>
      <button className="btn-glass px-4">Edit</button>
    </GlassCard>
  )
}
...
function Settings() {
  return (
    <div className="flex-1 overflow-y-auto p-6 bg-surface animate-fade-in">
      ...
      <h1 className="font-bold text-display-lg text-on-surface mb-1">Settings</h1>
      <p className="text-on-surface-variant text-body-md mb-8">Configure your personal and workspace preferences.</p>
